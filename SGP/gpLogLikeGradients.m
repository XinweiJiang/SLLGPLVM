function [gParam, gZ_u, gZ, g_beta] = gpLogLikeGradients(model, Z, M, Z_u)
  ...
  switch model.approx
   case 'ftc'
    ...
    if nargout > 2
      ...
      gKZ = kernGradX(model.kern, Z, Z);
      gKZ = gKZ*2;
      dgKZ = kernDiagGradX(model.kern, Z);
      for i = 1:model.N
        gKZ(i, :, i) = dgKZ(i, :);
      end
    ...
    if isfield(model, 'S')
      ...
      if nargout > 2
        ...
        for i = 1:model.N
          counter = counter + 1;
          for j = 1:model.q
            gZ(i, j) = gZ(i, j) + gKZ(:, j, i)'*gK(:, counter);
          end
        end
      end
      %%% Compute Gradients of Kernel Parameters %%%
      g_param = g_param + kernGradient(model.kern, Z, gK);
    else
      for k = 1:model.q
        ...
        if nargout > 2
          ...
          for i = ind
            counter = counter + 1;
            for j = 1:model.p
              gZ(i, j) = gZ(i, j) + gKZ(ind, j, i)'*gK(:, counter);
            end
          end
        end
        %%% Compute Gradients of Kernel Parameters %%%
