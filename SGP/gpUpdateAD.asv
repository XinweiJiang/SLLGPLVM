function model = gpUpdateAD(model, Z)

% GPUPDATEAD Update the representations of A and D associated with the model.
%
%	Description:
%
%	MODEL = GPUPDATEAD(MODEL, X) updates the representations of A and D
%	in the model when called by gpUpdateKernels.
%	 Returns:
...
% 	gpUpdateAD.m CVS version 1.2
% 	gpUpdateAD.m SVN version 498
...
switch model.approx
 ...
 case 'fitc'
  ...
  if ~isfield(model, 'isSpherical') | model.isSpherical
    model.diagD = 1 + model.beta*model.diagK ...        % =\Lambda
    ...
    model.A = 1/model.beta*model.K_uu + K_ufDinvK_uf;    %=Q
