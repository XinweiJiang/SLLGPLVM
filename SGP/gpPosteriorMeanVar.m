function [mu, varsigma] = gpPosteriorMeanVar(model, Z)
...
% 	gpPosteriorMeanVar.m CVS version 1.3
% 	gpPosteriorMeanVar.m SVN version 178
% 	last update 2009-01-08T13:48:45.000000Z
...
mu = zeros(size(Z, 1), model.q);
if nargout > 1
  varsigma = zeros(size(Z, 1), model.q);
end
...
while startVal <= size(Z, 1)
  indices = startVal:endVal;

  % Compute kernel for new point.
  switch model.approx
   case 'ftc'
    KX_star = kernCompute(model.kern, model.Z, Z(indices, :));  
   case {'dtc', 'dtcvar', 'fitc', 'pitc'}
    KX_star = kernCompute(model.kern, model.Z_u, Z(indices, :));  
  end
  ...
  if nargout > 1
    if ~isfield(model, 'isSpherical') || model.isSpherical
      ...
      diagK = kernDiagCompute(model.kern, Z(indices, :));
      switch model.approx
       case 'ftc'
      ...
      varsigma(indices, :) = repmat(varsig, 1, model.q);
    else
      diagK = kernDiagCompute(model.kern, Z(indices, :));
      for i = 1:model.d
