function g = tpsKernGradient(kern, x, varargin)
...
% 	rbfKernGradient.m CVS version 1.7
% 	rbfKernGradient.m SVN version 375
% 	last update 2009-06-02T22:01:41.000000Z


% The last argument is covGrad
if nargin < 4
  [k, sk, dist2xx] = tpsKernCompute(kern, x);
else
  [k, sk, dist2xx] = tpsKernCompute(kern, x, varargin{1});
end

sk = eye(size(x,1))+sk;

g(1, 1) = sum(sum(varargin{end}.*sk));
