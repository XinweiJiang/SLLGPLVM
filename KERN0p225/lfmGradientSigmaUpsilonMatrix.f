...
C ------ GATEWAY SUBROUTINE
	  ...
	  DOUBLE PRECISION RGRAD(ROWS, COLS), IGRAD(ROWS, COLS)
      COMPLEX*16 GAMMA
...
C		real(z1) >= 0 and real(z2)>=0
     ...
     $                  - GAMMA*(T1(I)-T2(J))) 
     $                  - 2*EXP(-((T1(I)-T2(J))**2/SIG2) 
     $                  + LOG(((T1(I)-T2(J))**2)*WOFZ1/(DEV**3) 
     $                  + ((T1(I)-T2(J))/SIG2+GAMMA/2)
     $                  * (1/SQRTPI	-Z1*WOFZ1))) 
     $                  - 2*EXP(-((T2(J)**2)/SIG2)-GAMMA*T1(I) 
     $                  + LOG((T2(J)**2)*WOFZ2(J)/(DEV**3) 
     $                  + (T2(J)/SIG2-GAMMA/2)*
     $                  ((1/SQRTPI)-Z2(J)*WOFZ2(J))))
              END IF
C		real(z1) < 0 and real(z2)>=0
		   IF(REALZ1. LT. 0. AND .REALZ2(J). GE .0  ) THEN
            GRAD =   2*EXP(-(T1(I)-T2(J))**2/SIG2 
     $                  + LOG(((T1(I)-T2(J))**2)*WOFZ1/(DEV**3) 
     $                  - ((T1(I)-T2(J))/SIG2+GAMMA/2)
     $                  * (1/SQRTPI	+Z1*WOFZ1))) 
     $                  - 2*EXP(-(T2(J)**2)/SIG2-GAMMA*T1(I) 
