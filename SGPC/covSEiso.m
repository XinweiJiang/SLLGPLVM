function [A, B] = covSEiso(loghyper, x, z, d)
...
elseif nargin == 3                                                      % compute derivative matrix
...
elseif nargin == 4
    if d > 0
        ...
        A2 = repmat(x(:,d),1,t)-repmat(z(:,d)',n,1);
        A = A1.*A2./(ell^2);
    else
        if d == -1                                                   % first parameter
            A = sf2*exp(-sq_dist(x'/ell,z'/ell)/2).*sq_dist(x'/ell,z'/ell);  
         else                                                       % second parameter
            A = 2*sf2*exp(-sq_dist(x'/ell,z'/ell)/2);  
         end
    end
end
