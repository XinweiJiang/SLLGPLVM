function [f, g] = fgplvmObjectiveGradient0(params, model)
...
options = model.modelR.options;
model.modelR = gpCreate(size(model.X0,2), model.p, model.X0, x, options);
...
if size(params, 1) > size(params, 2)
  ...
  transpose = true;
  model.modelL.kern.nParams
  model.modelR.kern.nParams
