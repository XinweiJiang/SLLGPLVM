function model = fgplvmCreate(p, d, X, Y, options)
...
%	  X - the data to be modelled in design matrix format (as many rows
...
if size(Y, 2) ~= d
  error(['Input matrix X does not have dimension ' num2str(d)]);
end

if isstr(options.initZ)
  initFunc = str2func([options.initZ 'Embed']);
  Z = initFunc(X, p);
else
  if size(options.initX, 1) == size(Y, 1) ...
        & size(options.initX, 2) == p
    Z = options.initX;
  else
...
model = gpCreate(p, d, X, Y, options);

model.type = 'fgplvm';
% model.gType = options.gType;
