function model = fgplvmExpandParam1(model, params)
...
% 	fgplvmExpandParam.m SVN version 773
% 	last update 2010-05-18T08:10:37.000000Z
...
if isfield(model, 'back') & ~isempty(model.back)
  ...
  % Reset the scales of the matrix W
  endVal = model.p*model.q;
  model.W = reshape(params(startVal:endVal), model.p, model.q);
  model.X = model.X0*model.W;
end
startVal = endVal+1;
