/* lnDiffErfs.c version 

NDLUTIL distribution version 0.162 published on Wednesday 19 May 2010 at 07:55
...
mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (!(((dim1_1 == 1 && dim2_1 == 1) || ((dim1_1 == dim1 && dim2_1 == dim2))) &&
    ...
    mexErrMsgTxt("lnDiffErfs: invalid input sizes");
  ...
  /* Return complex result instead of signs if just one output argument */
  if (nlhs < 2) {
    signs = mxGetPi(plhs[0]);
    for (i=0; i<dim1*dim2; i++) {
      signs[i] = 0.5 * (1 - signs[i]) * M_PI;
    }
  }
